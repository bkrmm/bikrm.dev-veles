...
class DashboardManager {
    constructor() {
        ...
        this.spendingByCategory = [
            ...
            { category: 'Transportation', amount: 450, percentage: 15.4, trend: -2.1 },
    ...
    initCharts() {
        ...
        const goalsData = {
            labels: ['Emergency Fund', 'Vacation', 'New Car', 'Home Down Payment', 'Retirement'],
            datasets: [
                {
                    label: 'Current',
        ...
        const reportsData = {
            ...
            datasets: [
                {
                    label: 'Income',
                    data: [4200, 4300, 4250, 4400, 4350, 4500, 4450, 4542, 4600, 4650, 4700, 4750],
                    borderColor: 'rgba(99, 102, 241, 0.8)',
                ...
                {
                    label: 'Expenses',
                    data: [2800, 2900, 2750, 2950, 2850, 2900, 2800, 2931, 2950, 3000, 3050, 3100],
                    borderColor: 'rgba(236, 72, 153, 0.8)',
                ...
                {
                    label: 'Savings',
                    data: [1400, 1400, 1500, 1450, 1500, 1600, 1650, 1611, 1650, 1650, 1650, 1650],
        ...
        const netWorthData = {
            ...
            datasets: [
                {
                    label: 'Assets',
                    data: [65000, 66500, 68000, 69500, 71000, 72500, 74000, 75500, 77000, 78500, 80000, 81500],
                    borderColor: 'rgba(16, 185, 129, 0.8)',
                    backgroundColor: 'rgba(16, 185, 129, 0.2)',
                    tension: 0.4,
                    fill: true
                ...
                {
                    label: 'Liabilities',
                    data: [42000, 41500, 41000, 40500, 40000, 39500, 39000, 38500, 38000, 37500, 37000, 36500],
                    borderColor: 'rgba(236, 72, 153, 0.8)',
                    backgroundColor: 'rgba(236, 72, 153, 0.2)',
                    tension: 0.4,
                    fill: true
                ...
                {
                    label: 'Net Worth',
                    data: [23000, 25000, 27000, 29000, 31000, 33000, 35000, 37000, 39000, 41000, 43000, 45000],
                    borderColor: 'rgba(99, 102, 241, 0.8)',
                    backgroundColor: 'rgba(99, 102, 241, 0.2)',
                    tension: 0.4,
                    fill: false,
        ...
        // Initialize charts
        this.charts.overview = new Chart(
        ...
        document.querySelector('#reports-content .dashboard-grid').appendChild(netWorthChartContainer);
        
        this.charts.netWorth = new Chart(
    ...
    generateTransactionData() {
        const categories = ['Housing', 'Transportation', 'Food', 'Utilities', 'Entertainment', 'Income', 'Other'];
        const descriptions = {
            'Housing': ['Rent', 'Mortgage', 'Property Tax', 'Home Insurance', 'Maintenance'],
            'Transportation': ['Gas', 'Car Payment', 'Car Insurance', 'Public Transit', 'Uber/Lyft'],
            'Food': ['Groceries', 'Restaurant', 'Coffee Shop', 'Food Delivery', 'Takeout'],
            'Utilities': ['Electricity', 'Water', 'Internet', 'Phone', 'Streaming Services'],
            'Entertainment': ['Movies', 'Concerts', 'Subscriptions', 'Hobbies', 'Sports'],
            'Income': ['Salary', 'Bonus', 'Freelance', 'Interest', 'Dividends'],
            'Other': ['Shopping', 'Gifts', 'Donations', 'Healthcare', 'Education']
        };
        
        const accounts = ['Checking Account', 'Credit Card', 'Savings Account', 'Investment Account'];
    ...
    renderTransactions(page = 1, limit = 10) {
        ...
        document.getElementById('pagination-start').textContent = sortedData.length > 0 ? start + 1 : 0;
        ...
        document.getElementById('pagination-next').disabled = end >= sortedData.length;
        ...
        paginatedData.forEach(transaction => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <span class="category-tag ${transaction.category.toLowerCase()}">${transaction.category}</span>
                </td>
    ...
    getFilteredTransactions() {
        ...
        return this.transactionData.filter(transaction => {
            ...
            if (searchTerm) {
                ...
                const accountMatch = transaction.account.toLowerCase().includes(searchTerm);
    ...
    getSortedTransactions(transactions) {
        ...
        return [...transactions].sort((a, b) => {
            ...
            } else if (column === 'description' || column === 'category' || column === 'account') {
    ...
    updateFinancialMetrics() {
        ...
        const overviewContent = document.getElementById('overview-content');
        const summarySection = overviewContent.querySelector('.dashboard-item.summary');
        ...
        metricsSection.innerHTML = `
            ...
            <div class="metrics-grid">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-content">
                        <div class="metric-value">${this.metrics.savingsRate}%</div>
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-content">
                        <div class="metric-value">${this.metrics.debtToIncome}</div>
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-content">
                        <div class="metric-value">${this.metrics.emergencyFund.percentage}%</div>
        ...
        summarySection.parentNode.insertBefore(metricsSection, summarySection.nextSibling);
    }
    ...
    updateCategoryBreakdown() {
        ...
        categorySection.innerHTML = `
            ...
            <table class="category-table">
                ...
                <tbody>
                    ${this.spendingByCategory.map(category => {
                        const trendIcon = category.trend > 0 ? '↑' : category.trend < 0 ? '↓' : '→';
                        const trendClass = category.trend > 0 ? 'negative' : category.trend < 0 ? 'positive' : 'neutral';
                        
                        return `
                            <tr>
                                <td>
                                    <span class="category-tag ${category.category.toLowerCase()}">${category.category}</span>
                                </td>
                                <td>$${category.amount.toLocaleString()}</td>
                                <td>${category.percentage}%</td>
                                <td class="${trendClass}">${Math.abs(category.trend)}% ${trendIcon}</td>
                                <td>
    ...
    initEventListeners() {
        ...
        document.querySelectorAll('.transaction-table th.sortable').forEach(th => {
            th.addEventListener('click', () => {
                ...
                th.querySelector('.sort-icon').textContent = this.sortConfig.direction === 'asc' ? '↑' : '↓';
